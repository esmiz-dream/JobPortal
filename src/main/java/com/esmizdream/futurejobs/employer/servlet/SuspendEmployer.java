 ...
 * @author hp
 */
public class SuspendEmployer extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SuspendEmployer</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SuspendEmployer at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equalsIgnoreCase("suspend")) {
            status = useraccountstatus.suspend(Integer.parseInt(request.getParameter("id")), "employer");
            if (status == 1) {
            ...
            status = useraccountstatus.unsuspend(Integer.parseInt(request.getParameter("id")), "employer");
            if (status == 1) {
